...
- **Skill tree — Ragnarok-style graph** (1 skill point/Job Lv; career caps **15 / 40 / 50**): the Skills panel draws native SVG prerequisite paths beneath semantic HTML nodes, with learned, learnable, preview, locked, and incompatible-path states. Both final-job branches stay visible; the rejected branch remains sealed. Rich tooltips, hotbar dragging, the stat-growth radar, branch identity, and combat guide remain intact. Defined in `js/progression.js` / `js/combat.js`.
- **Independent class choices via advancement quests**: Base Lv 15 unlocks one of two permanent second jobs; Base Lv 40 independently unlocks one of two third jobs, giving four combinations per base class and 14 unique final jobs. Second jobs retain their signature skill/mechanic, while third jobs own exclusive active/passive gates. Promotion raises the active Job cap without resetting Job Level or partial XP; XP earned while capped is discarded.
...
- **Job XP bar**: a thin blue bar rides above the gold base-XP bar and displays the active career cap: Job Lv15, 40, or 50.
...
- **28 animated monsters** incl. six guardians/bosses and five annex species—Mossback Beetle, Ruin Crawler, Tundra Lurker, Ashwing, and Constellation Mite—with distinct six-frame pixel data and depth-based habitats.
...
| Content Designer | `js/content.js` | 28 monsters, 83 items, 10 NPCs, 35-quest playthrough, story |
